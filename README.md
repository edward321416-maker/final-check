...
After profile confirmation, choose "자동 검사 계획 생성". The isolated planner sees the confirmed source/profile only; it never sees submission data. The deterministic gate authorizes only closed, source-grounded plans. `generic_review` remains REVIEW/EXTERNAL only, while `generic_verifier` may issue evidence-backed PASS/BLOCKER from actual bytes. Conditional, ambiguous, unsupported and failed checks remain REVIEW. Current MVP automatic file support: PDF / MP4.
