...
1. Open the ngrok HTTPS URL in a fresh browser and pass the one-time ngrok warning if it appears.
...
Classify the public ngrok URL, Codex calls, ffprobe, and actual media validation as ACTUAL. Unit/integration/browser safety tests are SELF. Injected failure tests are SIMULATED. Cloud failover, production SLA, DDoS resilience, large traffic, Vision/OCR, new formats, auth, payments, and backups are NOT TESTED.
