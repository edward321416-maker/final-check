...
export function HomeScreen() {
  ...
  return <><section className="hero"><div className="hero-copy"><span className="eyebrow"><span className="dot" /> AI SUBMISSION PREFLIGHT</span>
    ...
    <p className="demo-caption">원본 Validator v1.5가 실제 예시 파일을 검사합니다. 내 공고는 로컬 규칙 후보를 직접 검토합니다. AI 모델·Vision은 미연결입니다.</p>
...
export function ResultsScreen() {
  ...
  return <Guard requireResults><ModeNote /><PageTitle step="03 / PREFLIGHT RESULTS" title="근거를 확인하고, 제출을 준비하세요" description="판정별 근거와 필요한 조치를 확인한 뒤 수정한 파일로 다시 검사할 수 있습니다." />
    ...
    </section><div className="results-footer"><span>{session?.validation_profile === "generic" ? "사용자가 확정한 요구사항입니다. 자동 검증은 미지원이며 PASS/BLOCKER를 생성하지 않았습니다." : "원본 Validator v1.5의 실제 검사 결과입니다. REVIEW 항목과 최종 제출은 직접 확인하세요."}</span><Link href="/recheck" className="text-link">수정 패키지 재검사 →</Link></div>
  </Guard>;
}
